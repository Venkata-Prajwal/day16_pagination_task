...
var index = 1,offSet;
var slicedItems;var html,pages;
const displayPageNav = perPage => {
  ...
  if(index >=  dataSet.length/perPage){
  ...
  let last = 10;
  pagination += `<a href="#"  onClick="displayItems(${last},10)">Last</a>`
...
const showNextorPrev = (page,noofrows,opt) => {
  if(opt == 'prev'){
    ...
    slicedItems = dataSet.slice(pagedata, offnewset)
  }
  else
  {
    if(index < noofrows){}
    ...
    slicedItems = dataSet.slice(pagedata, newoffset)
  }
  displayPageNav(10);
  html = slicedItems.map(item => 
   `<tr>
      <td>${item.id}</td>
      <td>${item.name}</td>
      <td>${item.email}</td>
...
const displayItems = ( page = 1, perPage = 2 ) => {
  ...
  if(page == 1 || page <=0)  {
    index = page
    offSet = perPage
    slicedItems = dataSet.slice(0, offSet)
    ...
    slicedItems = dataSet.slice(strtindex, offSet)
  }
